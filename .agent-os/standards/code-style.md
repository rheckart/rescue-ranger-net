...
<conditional-block task-condition="c#" context-check="c#-style">
IF current task involves writing or updating C#:
  IF csharp-style.md already in context:
    SKIP: Re-reading this file
    NOTE: "Using C# style guide already in context"
  ELSE:
    <context_fetcher_strategy>
      IF current agent is Claude Code AND context-fetcher agent exists:
        USE: @agent:context-fetcher
        REQUEST: "Get C# style rules from code-style/csharp-style.md"
        PROCESS: Returned style rules
      ELSE:
        READ: @~/.agent-os/standards/code-style/csharp-style.md
    </context_fetcher_strategy>
ELSE:
  SKIP: C# style guide not relevant to current task
</conditional-block>
